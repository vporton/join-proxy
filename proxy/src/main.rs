...
async fn serialize_http_response(
    response: reqwest::Response,
) -> anyhow::Result<(Vec<u8>, Vec<u8>)> {
    let headers_list = response
        .headers()
        .into_iter()
        .map(|(k, v)| -> anyhow::Result<String> { Ok(k.to_string() + "\t" + v.to_str()?) })
    ...
    Ok((
        [header_part.as_bytes(), b"\n", &bytes].concat(),
        bytes.to_vec(),
    )) // TODO: `bytes` is passed two times.
}
...
fn deserialize_http_response(data: &[u8]) -> anyhow::Result<actix_web::HttpResponse<Vec<u8>>> {
    ...
    let mut response =
        actix_web::HttpResponse::with_body(StatusCode::from_u16(status_code)?, Vec::from(body));
    ...
    for header_str in headers_bytes.split(|&c| c == b'\r') {
        ...
        headers.append(
            http_for_actix::HeaderName::from_bytes(k)?,
            http_for_actix::HeaderValue::from_bytes(v)?,
        );
    }
...
async fn proxy(
    ...
    let path = req
        .uri()
        .path_and_query()
        .ok_or(anyhow!("can't get path and query"))?
        .as_str();
    ...
    if let Some(our_secret) = &config.our_secret {
        let passed_key = req
            ...
            .map(|v| {
                v.to_str()
                    .map_err(|_| anyhow!("Cannot read header X-JoinProxy-Key"))
            })
            .transpose()?;
    ...
    let mut cache_lock = cache
        .lock(&Vec::from(actix_request_hash.as_slice()))
        .await?;
    ...
    if let Some(serialized_response) = (*cache_lock).inner().await {
        ...
        if let (Some(agent), Some(callback)) = (&state.agent, &config.callback) {
            info!("Callback...");
            let res = agent
                .update(&callback.canister, &callback.func)
                .with_arg(Encode!(&actix_request_hash.as_slice())?)
                .call_and_wait()
                .await;
            match res {
                Ok(res) => {
        ...
        if config.require_x_principal && caller_principal.is_none() {
            // FIXME: Is `Some(true)` correct?
            return Err(anyhow!("missing X-Principal header").into());
        }
        // TODO: The below line is a hack.
        let serve_config_uid_s = if let Some(serve_config_uid) = req.headers().get("x-config") {
            // TODO: Use the last header, remove it.;
        ...
        let (actix_response, reqwest_response) = if let Some((serve_config_uid, caller_principal)) =
            serve_config_uid_s
        {
            let serve_config_uid_raw = serve_config_uid.to_str()?;
            let serve_config_uid =
                hex::decode(serve_config_uid_raw).map_err(|_| anyhow!("broken hex ID"))?;
            ...
            let (
                ...
                .get_result::<(i64, i32, bool, bool /*, i32, i32, i32*/)>(
                    &mut *state.conn.lock().await,
                )
                .map_err(|_| anyhow!(format!("no serve config with uid {serve_config_uid_raw}")))?;
            let a_user_principal: Principal = users
                .filter(self::schema::users::dsl::id.eq(a_user_id))
            ...
            let additional_request_headers = add_request_headers
                .filter(
                    self::schema::add_request_headers::dsl::server_setup_id.eq(a_server_setup_id),
                )
                .select((
            ...
            let request_headers_to_remove = remove_request_headers
                .filter(
                    self::schema::remove_request_headers::dsl::server_setup_id
                        .eq(a_server_setup_id),
                )
                .select(self::schema::remove_request_headers::dsl::header_name)
            ...
            let request_headers = http::HeaderMap::from_iter(
                request_headers
                    .map(|h| -> MyResult<_> {
                        Ok((
                            http::HeaderName::from_str(h.0.as_str())
                                .map_err(|_| InvalidHeaderNameError::default())?,
                            http::HeaderValue::from_str(h.1.to_str()?)
                                .map_err(|_| InvalidHeaderValueError::default())?,
                        ))
                    })
                    .into_iter()
            ...
            let builder = state
                .client
                .request(method, req.uri().to_string())
            ...
            // TODO: duplicate block of code
            let mut actix_response = actix_web::HttpResponse::new(StatusCode::from_u16(status)?);
            let response_headers = actix_response.headers_mut();
            for (k, v) in reqwest_response.headers() {
                response_headers.append(
                    http_for_actix::HeaderName::from_str(k.as_str())
                        .map_err(|_| InvalidHeaderNameError::default())?,
                    http_for_actix::HeaderValue::from_str(v.to_str()?)
                        .map_err(|_| InvalidHeaderValueError::default())?,
                );
            }
            ...
            let response_headers_to_remove = remove_response_headers
                .filter(
                    self::schema::remove_response_headers::dsl::server_setup_id
                        .eq(a_server_setup_id),
                )
                .select(self::schema::remove_response_headers::dsl::header_name)
                .get_results::<String>(&mut *state.conn.lock().await)
                .map_err(|_| anyhow!(format!("cannot read DB")))?
                .into_iter();
            ...
            for k in hop_by_hop
                .into_iter()
                .map(|s| Ok(http_for_actix::HeaderName::from_static(s)))
                .chain(response_headers_to_remove.map(|s| {
                    http_for_actix::HeaderName::from_str(&s)
                        .map_err(|_| InvalidHeaderNameError::default().into())
                }))
            ...
            let response_headers_to_add = add_response_headers
                .filter(
                    self::schema::add_response_headers::dsl::server_setup_id.eq(a_server_setup_id),
                )
                .select((
            ...
            for (k, v) in response_headers_to_add {
                response_headers.append(
                    http_for_actix::HeaderName::from_str(&k)
                        .map_err(|_| InvalidHeaderNameError::default())?,
                    http_for_actix::HeaderValue::from_str(&v)
                        .map_err(|_| InvalidHeaderValueError::default())?,
                );
            }
            ...
            let request_headers = req
                .headers()
                .into_iter()
                .filter(|h| h.0 == http_for_actix::HeaderName::from_static("host"));
            let request_headers = http::HeaderMap::from_iter(
                request_headers
                    .map(|h| -> MyResult<_> {
                        Ok((
...
async fn main() -> anyhow::Result<()> {
    ...
    let proxy_server_url =
        config.bind_proxy.host.clone() + ":" + config.bind_proxy.port.to_string().as_str();
    let api_server_url =
        config.bind_api.host.clone() + ":" + config.bind_api.port.to_string().as_str(); // TODO: duplicate code

    ring::default_provider().install_default().unwrap();

    let cache = Arc::new(Mutex::new(Box::<BinaryCache>::from(Box::new(
    ...
    let database_url = config.db.clone();
    ...
    let oauth_state = auth::State::preconfigured(
        challenge_store.clone(),
        agent2.as_ref().unwrap().read_root_key(), // FIXME@P1: Why `unwrap` here?
    )
    .start(); // FIXME@P1: Why `unwrap` here?
    ...
    let proxy_server = HttpServer::new(move || {
        ...
        let state = State {
            ...
            agent: agent.clone(),
            conn: tokio::sync::Mutex::new(
                PgConnection::establish(&database_url).expect("DB connection"),
            ),
        };
        App::new().service(
            web::scope("")
    ...
    info!(
        "Starting Proxy at {} (https={})",
        proxy_server_url, is_https
    );
    let proxy = if is_https {
        if let (Some(cert_file), Some(key_file)) = (cert_file, key_file) {
            let cert_file =
                &mut BufReader::new(File::open(cert_file).context("Can't read HTTPS cert.")?);
            let key_file =
                &mut BufReader::new(File::open(key_file).context("Can't read HTTPS key.")?);
            ...
            let key = pkcs8_private_keys(key_file)
                .next()
                .transpose()?
                .ok_or(anyhow!("No private key in the file."))?;
