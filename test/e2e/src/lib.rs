...
mod tests {
    ...
    impl<'a> OurDFX<'a> {
        pub async fn new(
            ...
            run_successful_command(
                &mut Command::new("/root/.local/share/dfx/bin/dfx")
                    .args(
                        [
                            &[
                                "start",
            ...
            run_successful_command(
                Command::new("/root/.local/share/dfx/bin/dfx")
                    .args(["deploy"])
                    .current_dir(base.dir.path().join("motoko")),
            )?;
            ...
            let test_canister_id = canister_ids.as_object().unwrap()["test"]
                .as_object()
                .unwrap()["local"]
                .as_str()
                .unwrap();
            let call_canister_id = canister_ids.as_object().unwrap()["call"]
                .as_object()
                .unwrap()["local"]
                .as_str()
                .unwrap();
            ...
            agent
                .fetch_root_key()
                .await
                .context("Fetching root keys.")?; // DON'T USE this on mainnet
    ...
    impl<'a> Drop for OurDFX<'a> {
        fn drop(&mut self) {
            run_successful_command(
                &mut Command::new("/root/.local/share/dfx/bin/dfx")
                    .args(["stop"])
                    .current_dir(self.base.dir.path().join("motoko")),
            )
            .context("Stopping DFX")
            .expect("can't stop DFX");
        }
    }
    ...
    async fn test_calls<'a>(
        test: &'a OurDFX<'a>,
        path: &str,
        arg: &str,
        body: &str,
    ) -> Result<Vec<HttpHeader>, Box<dyn std::error::Error>> {
        let res = test
            .agent
            .update(&test.test_canister_id, "test")
            .with_arg(Encode!(&path, &arg, &body, &":8443", &":8081")?)
            .call_and_wait()
            .await
            .context("Call to IC.")?;
        let (text, headers) =
            Decode!(&res, String, Vec<HttpHeader>).context("Decoding test call response.")?;
        assert_eq!(text, format!("path={}&arg={}&body={}", path, arg, body),);
        Ok(headers)
    }
    ...
    impl MyTest {
        pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
            ...
            let _test_http = TemporaryChild::spawn(
                &mut Command::new(
                    test.workspace_dir
                        .join("target")
                        .join("debug")
    ...
    async fn test_artificial_delay() -> Result<(), Box<dyn std::error::Error>> {
        {
            ...
            let _proxy = TemporaryChild::spawn(
                &mut Command::new(
                    mytest
                        .test
                        .workspace_dir
                ...
                .args(["-c", "config.toml"])
                .current_dir(mytest.test.dir.path()),
                Capture {
                    stdout: None,
                    stderr: None,
                },
            )
            .context("Running Joining Proxy")?;
            run_successful_command(
                Command::new("/root/.local/share/dfx/bin/dfx")
                    .args(["deploy"])
                    .current_dir(mytest.test.dir.path().join("motoko")),
            )?;
    ...
    async fn test_loading_new() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for headers in [&headers1, &headers2, &headers3] {
            if headers
                .iter()
                .any(|h| h.name == "x-joinproxy-response" && h.value == "Hit")
            {
                hit_count += 1;
            }
            if headers
                .iter()
                .any(|h| h.name == "x-joinproxy-response" && h.value == "Miss")
            {
                miss_count += 1;
            }
        }
    ...
    async fn test_port_443() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _test_http2 = TemporaryChild::spawn(
            &mut Command::new(
                mytest
                    .test
                    .workspace_dir
        ...
        .context("Running mytest.test HTTPS server")?;
        sleep(Duration::from_millis(1000)).await; // Wait till daemons start.

        // Check https://local.vporton.name vs https://local.vporton.name:443
        let res = dfx
            .agent
            .update(&dfx.test_canister_id, "test")
            .with_arg(Encode!(&"/headers", &"", &"", &"", &"")?)
            .call_and_wait()
            .await
            .context("Call to IC 2.")?;
        let (text, _headers) =
            Decode!(&res, String, Vec<HttpHeader>).context("Decoding test call response.")?;
        assert!(text.contains("host: local.vporton.name\n"),);
        // local.vporton.name:443
        let res = dfx
            .agent
            .update(&dfx.test_canister_id, "test")
            .with_arg(Encode!(&"/headers", &"", &"", &":443", &":443")?)
            .call_and_wait()
            .await
            .context("Call to IC 2.")?;
        let (text, _headers) =
            Decode!(&res, String, Vec<HttpHeader>).context("Decoding test call response.")?;
        assert!(text.contains("host: local.vporton.name:443\n"),);
        drop(dfx);
        drop(mytest);
